                ...
                <div className="row ">
                    <div className="col-12 col-xl-5 col-lg-5 col-md-5">
                        <div className="section_details_padding">
                            <div className="looking-for-a-project">{!this.state.isLogin || (this.state.isLogin && this.state.isAdmin) ? "Looking for a part-time or project based talent?" : `Your profile is ${this.state.profilePer}% completed.`}</div>
                            {(!this.state.isLogin || (this.state.isLogin && this.state.isAdmin))  && <div className="work-with-the-best-freelance">Work with the best freelance talent from around the world on our secure,
                                flexible and cost-effective platform.</div>}
                            {(this.state.isLogin && !this.state.isAdmin) && <ProgressBar colorShift={false} borderColor="black" fillColor="#ff6158" percent={this.state.profilePer} />}
                        </div>
                        {(!this.state.isLogin || (this.state.isLogin && this.state.isAdmin)) && <div><button className="btn btn-primary btn-orange-color my-2 my-sm-0 get_started" onClick={this.getStarted} type="button">Get Started</button><button className="btn btn-primary btn-orange-color my-2 my-sm-0 get_started" style={{ marginLeft: "10px" }} onClick={() => { this.props.history.push("/analytics") }} type="button">Analytics</button></div>}
                        {(this.state.isLogin && !this.state.isAdmin) && <div style={{ marginTop: "20px" }}><button className="btn btn-primary btn-orange-color my-2 my-sm-0 get_started" onClick={this.findJob} type="button">Find job for me</button><button style={{ marginLeft: "20px" }} className="btn btn-primary btn-orange-color my-2 my-sm-0 get_started" onClick={this.gotoProfile} type="button">Update profile</button></div>}
                    </div>
